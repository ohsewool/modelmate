...
- [ ] 토큰을 쓰지 않고 연습할 때는 `?presenter=1` 발표자 세션으로 들어간 뒤 상단 상태 배지를 눌러 `데모 모드`로 바꾼다.
- [ ] 실제 Gemini 호출을 다시 쓰려면 발표자 세션에서 상단 상태 배지를 다시 눌러 `정상 작동`으로 바꾼다.
...
- 일반 URL에서는 데모 토글이 보이지 않는다.
- 발표자 URL로 들어간다: `https://web-production-5d6fa.up.railway.app/upload?presenter=1`
- 발표자 세션에서 상단의 `정상 작동` 배지를 클릭해 `데모 모드`로 바꾼다.
- URL 방식도 가능하다: `https://web-production-5d6fa.up.railway.app/upload?presenter=1&demo=1`
- 발표자용 데모 파일 안내 카드는 일반 `/upload`에는 보이지 않고, `/upload?presenter=1`에서만 보인다.
...
- URL 방식도 가능하다: `https://web-production-5d6fa.up.railway.app/upload?presenter=0`
