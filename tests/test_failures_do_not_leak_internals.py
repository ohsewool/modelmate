...
class TestNoHandlerHandsBackARawException:
    ...
    def test_the_convention_is_actually_used(self):
        ...
        **정확한 수에서 하한선으로 바꿨다 — 판단을 뒤집는다.**

        처음엔 `== 12`로 박았다. 이 저장소는 하한선을 여러 번 비판해 왔다(하한선은
        하나가 빠지는 것을 못 본다). 그런데 여기서는 두 회차 연속으로 **좋은 변경에
        빨간불이 켜졌다** — 409 하나를 더할 때, 그리고 400 하나를 더할 때. 고칠 것은
        매번 숫자 하나였다.

        하한선을 비판한 근거를 여기에 대보면 성립하지 않는다. "하나가 빠지는 것"은
        **바로 위 `test_there_are_none`이 잡는다** — `failure_detail`이 원시 문자열로
        바뀌면 그 검사가 걸린다. 정확한 수가 추가로 지키는 것은 없고, 막는 것은
        관례를 넓히는 변경뿐이다.
        ...
        **다른 검사가 그 실패를 이미 덮고 있으면 정확한 수는 값이 아니라 마찰이다.**
        ...
        assert source.count("failure_detail(") >= 13, (
            "`failure_detail` 호출이 줄었다. 무엇이 그 자리를 대신하는지 보라 — "
            "원시 문자열이면 위 `test_there_are_none`도 함께 빨간불이어야 한다.")
