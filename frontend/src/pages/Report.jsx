        ...
        <div className="animate-slide-up" style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div className="card">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
              <div style={{ borderRadius:12, padding:16, textAlign:'center', border:'1px solid var(--border)', background:'var(--bg)' }}>
                <p style={{ fontSize:10, color:'var(--text-2)', marginBottom:8, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.1em', margin:'0 0 8px' }}>최고 모델</p>
                <p style={{ color:'var(--text)', fontWeight:700, margin:0 }}>{state.best_model?.split(' ')[0]}</p>
              </div>
              <div style={{ borderRadius:12, padding:16, textAlign:'center', border:'1px solid rgba(99,102,241,0.2)', background:'rgba(99,102,241,0.06)' }}>
                <p style={{ fontSize:10, color:'var(--text-2)', marginBottom:8, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.1em', margin:'0 0 8px' }}>ROC-AUC</p>
                <p style={{ color:'#4f46e5', fontWeight:700, fontSize:22, fontVariantNumeric:'tabular-nums', margin:0 }}>{state.cv_results?.[0]?.roc_auc ?? '—'}</p>
              </div>
              <div style={{ borderRadius:12, padding:16, textAlign:'center', border:`1px solid ${state.optuna_result ? 'rgba(16,185,129,0.3)' : 'var(--border)'}`, background: state.optuna_result ? 'rgba(16,185,129,0.06)' : 'var(--bg)' }}>
                <p style={{ fontSize:10, color:'var(--text-2)', marginBottom:8, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.1em', margin:'0 0 8px' }}>Optuna</p>
                <p style={{ fontWeight:700, color: state.optuna_result ? '#059669' : 'var(--text)', margin:0 }}>
