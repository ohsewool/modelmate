          ...
          <table className="data-table">
            ...
            <tbody>{reports.map((report, index) => (
              <tr key={`${report.project?.id}-${report.report_id || index}`}>
                ...
                <td>{report.project?.id ? <Link to={`/projects/${report.project.id}?tab=report`}>View report</Link> : <button className="btn-secondary" onClick={() => nav('/report')}>View report</button>}</td>
              </tr>
            ))}</tbody>
          </table>
        </section>
