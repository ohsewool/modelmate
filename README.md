...
남은 스물하나는 **이름으로 뒀다.** 대부분은 표를 만드는 부팅 시 DDL(`ensure_*`)이고, 거기서 예외가 나면 부팅이 실패해 프로세스가 어차피 못 뜬다. 나머지는 로그인한 사용자만 도달한다. 고치는 것이 맞지만 **어디가 남았는지 아는 채로 두는 것이 먼저다** — 목록이 정확히 일치해야 하므로 새로 생기는 것도, 고쳐서 빠지는 것도 걸린다.
...
```bash
python3 -m pytest tests/ -q          # 912 tests
```
