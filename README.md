...
**스물둘을 한 번에 고치지 않았다.** 배포된 앱의 함수 스물둘을 기계적으로 감싸는 것은 이 회차에 감당할 위험이 아니고, 각 함수의 조기 반환·예외 경로가 제각각이다. **인증 없이 도달하는 쓰기 넷**을 고쳤다 — `auth_google`(앞 절), `auth_signup`, `auth_login`, `auth_logout`. 바깥에서 입력을 넣어 예외를 만들 수 있는 자리다.
...
남은 열여덟은 **이름으로 뒀다.** 대부분은 표를 만드는 부팅 시 DDL(`ensure_*`)이고, 거기서 예외가 나면 부팅이 실패해 프로세스가 어차피 못 뜬다. 나머지는 로그인한 사용자만 도달한다. 고치는 것이 맞지만 **어디가 남았는지 아는 채로 두는 것이 먼저다** — 목록이 정확히 일치해야 하므로 새로 생기는 것도, 고쳐서 빠지는 것도 걸린다.
...
```bash
python3 -m pytest tests/ -q          # 893 tests
```
